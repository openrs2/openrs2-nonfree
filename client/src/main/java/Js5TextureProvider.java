...
public class Js5TextureProvider implements TextureProvider {
	...
	protected Js5TextureProvider(@OriginalArg(1) Js5 texturesArchive, @OriginalArg(2) Js5 spritesArchive, @OriginalArg(0) Js5 materialsArchive) {
		...
		@Pc(28) int materials = buffer.readUnsignedShort();
		this.materials = new Material[materials];
		for (@Pc(34) int i = 0; i < materials; i++) {
			if (buffer.readUnsignedByte() == 1) {
				this.materials[i] = new Material();
			}
		}
		for (@Pc(60) int i = 0; i < materials; i++) {
			if (this.materials[i] != null) {
				this.materials[i].aBoolean237 = buffer.readUnsignedByte() == 0;
			}
		}
		for (@Pc(88) int i = 0; i < materials; i++) {
			if (this.materials[i] != null) {
				this.materials[i].aBoolean235 = buffer.readUnsignedByte() == 1;
			}
		}
		for (@Pc(117) int i = 0; i < materials; i++) {
			if (this.materials[i] != null) {
				this.materials[i].lowDetail = buffer.readUnsignedByte() == 1;
			}
		}
		for (@Pc(149) int i = 0; i < materials; i++) {
			if (this.materials[i] != null) {
				this.materials[i].aBoolean233 = buffer.readUnsignedByte() == 1;
			}
		}
		for (@Pc(180) int i = 0; i < materials; i++) {
			if (this.materials[i] != null) {
				this.materials[i].aByte10 = buffer.readByte();
			}
		}
		for (@Pc(206) int i = 0; i < materials; i++) {
			if (this.materials[i] != null) {
				this.materials[i].aByte7 = buffer.readByte();
			}
		}
		for (@Pc(232) int i = 0; i < materials; i++) {
			if (this.materials[i] != null) {
				this.materials[i].aByte8 = buffer.readByte();
			}
		}
		for (@Pc(254) int i = 0; i < materials; i++) {
			if (this.materials[i] != null) {
				this.materials[i].aByte9 = buffer.readByte();
			}
		}
		for (@Pc(277) int i = 0; i < materials; i++) {
			if (this.materials[i] != null) {
				this.materials[i].aShort26 = (short) buffer.readUnsignedShort();
			}
		}
		if (buffer.position < buffer.bytes.length) {
			for (@Pc(315) int i = 0; i < materials; i++) {
				if (this.materials[i] != null) {
					buffer.readByte();
				}
			}
			for (@Pc(337) int i = 0; i < materials; i++) {
				if (this.materials[i] != null) {
					buffer.readByte();
				}
			}
			for (@Pc(355) int i = 0; i < materials; i++) {
				if (this.materials[i] != null) {
					buffer.readUnsignedByte();
				}
			}
			for (@Pc(377) int i = 0; i < materials; i++) {
				if (this.materials[i] != null) {
					this.materials[i].aBoolean236 = buffer.readUnsignedByte() == 1;
				}
			}
			for (@Pc(406) int i = 0; i < materials; i++) {
				if (this.materials[i] != null) {
					buffer.readByte();
				}
			}
			for (@Pc(424) int i = 0; i < materials; i++) {
				if (this.materials[i] != null) {
					buffer.readUnsignedByte();
				}
			}
			for (@Pc(445) int i = 0; i < materials; i++) {
				if (this.materials[i] != null) {
					buffer.readUnsignedByte();
				}
			}
			for (@Pc(467) int i = 0; i < materials; i++) {
				if (this.materials[i] != null) {
					buffer.readUnsignedByte();
				}
			}
	...
	public final int[] method415(@OriginalArg(2) int id, @OriginalArg(0) int arg0, @OriginalArg(1) float arg1, @OriginalArg(3) int arg3) {
		return this.getTexture(id).method2653(arg1, this.materials[id].aBoolean236, this.spritesArchive, this, arg0, false, arg3);
	}
